...
const jobsUrl = computed(() => {
  return `/jobs?limit=${limit.value}&offset=${(page.value - 1) * limit.value}${state.value !== null ? `&state=${jobStateMapping[state.value]}` : ""}`;
});
watch(jobsUrl, () => {
...
const {
  ...
  pending: loadingJobs,
  refresh: refreshJobs,
} = await useAPI(jobsUrl, { watch: [jobsUrl] });

// Fetch jobs every 30 seconds with cleanup
const { pause: pauseJobsPolling, resume: resumeJobsPolling } = useIntervalFn(
  refreshJobs,
  30000,
);
