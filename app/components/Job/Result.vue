<template>
  <div class="result-tab-container">
    <div class="result-content-wrapper" ref="resultWrapper">
      ...
      <div v-if="ipfsResult?.opStates" class="is-family-monospace has-background-black has-text-white box light-mode result-box job-result-container result-with-counter" ref="resultContainer">
      ...
      <div v-else-if="ipfsJob?.ops" class="is-family-monospace has-background-black has-text-white box result-box job-result-container result-with-counter" ref="resultContainer">
      ...
      <div v-else class="is-family-monospace has-background-black has-text-white box result-box job-result-container result-with-counter" ref="resultContainer">
        <div class="row-count">
          <span>No logs available</span>
        </div>
      </div>
    ...
    <FullscreenModal :isOpen="resultModal.isOpen.value" title="Job Results" @close="resultModal.close">
      <div v-if="ipfsResult?.opStates" class="is-family-monospace has-background-black has-text-white box light-mode result-box job-result-container fullscreen-viewer result-with-counter" ref="fullscreenResultContainer">
        <template v-if="hasAnyContent(ipfsResult?.opStates || [])">
          <div v-for="opState in (ipfsResult?.opStates || [])" :key="opState.operationId">
            <div class="row-count has-text-link">
              <span>- Executed step '{{ opState.operationId }}'</span>
      ...
      <div v-else-if="ipfsJob && ipfsJob.ops" class="is-family-monospace has-background-black has-text-white box result-box job-result-container fullscreen-viewer result-with-counter" ref="fullscreenResultContainer">
        <template v-if="hasLegacyContent(ipfsResult, ipfsJob)">
          <div v-for="jobName in (ipfsJob.ops.find((j: any) => j.id === 'checkout')
          ? []
          : ['checkout']
      ...
      <div v-else class="is-family-monospace has-background-black has-text-white box result-box job-result-container fullscreen-viewer result-with-counter" ref="fullscreenResultContainer">
        <div class="row-count">
          <span>No logs available</span>
        </div>
      </div>
...
const hasLegacyContent = (ipfsResult: any, ipfsJob: any): boolean => {
  ...
  const jobNames = (ipfsJob.ops.find((j: any) => j.id === 'checkout')
    ? []
    : ['checkout']
  ).concat(ipfsJob.ops.map((j: any) => j.name || j.id));
  return jobNames.some((jobName: string) => {
    return ipfsResult.results && ipfsResult.results[jobName] && !jobName.endsWith('-volume');
  });
};
...
const getOpError = (opState: OpState): string | null => {
  ...
  if (opState.errors && Array.isArray(opState.errors) && opState.errors.length > 0) {
