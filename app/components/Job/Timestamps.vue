          ...
          <div class="is-flex is-align-items-center">
            <h2 class="title is-5 mb-0">
              ...
              <small class="ml-2">
                ...
                <span v-else-if="time == (365 / 12) * 24 * 3600"
                  >(last month)</span
                >
                <span v-else-if="time == (365 / 4) * 24 * 3600"
                  >(last quarter)</span
                >
...
const timestampsUrl = computed(() => {
  return `/jobs/stats/timestamps?period=${time.value}`;
});
watch(timestampsUrl, () => {
  timestamps.value = null;
});

const { data: timestamps, pending: loadingTimestamps } = await useAPI(
  timestampsUrl,
  { watch: [timestampsUrl] },
);
...
const totalDeployments = computed(() => {
  const data: Array<{ x: any; y: number }> =
    timestamps.value && timestamps.value.data ? timestamps.value.data : [];
  if (!data || data.length === 0) {
    return 0;
  }
  // Ensure 'y' exists and is a number before summing
  return data.reduce(
    (sum, point) => sum + (point && typeof point.y === "number" ? point.y : 0),
    0,
  );
});
